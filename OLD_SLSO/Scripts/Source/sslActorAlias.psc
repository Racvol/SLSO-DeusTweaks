...
sslActorStats Stats
SOS_API property SosApi auto
Actor PlayerRef
Faction SOS_SchlongifiedFaction
...
int Gender
int EdgeStack
int LastMentalDamage
Float LastMentalDamageTime
Float TemperMult
Float SchlongPercentEfficiency
bool isRealFemale
bool IsMale
bool IsFemale
bool IsCreature
...
float function GetScale()
	float output = 1.0
	if Config.RaceAdjustments && BaseRef
		output = BaseRef.GetHeight()
		If JSONUtil.IsGood("../SexLab/RaceGenderScales.json")
...
state Animating
	...
	event OnUpdate()
		...
		;	return don't work on events
			...
			; Сканируем зрителей раз в 5 секунд, а не каждый кадр
			if iCfg_Exhib_Mode > 0 && !IsCreature
...
; --- ДОБАВИТЬ В НАЧАЛО (Кэш переменных) ---
bool bSLSO_Initialized = false
int iCfg_SLA_Mode
int iCfg_Exhib_Mode
int iCfg_Masturb_Mode
int iCfg_Victim_Orgasm_Mode
int iCfg_Schlong_Bonus
int iCfg_Schlong_Med
float fCfg_Schlong_Curve
bool bCfg_Creature_Bonus
bool bCfg_Schlong_Oral
bool bCfg_MindBreak_Schlong
float fCfg_Estrus_Mod
float fLastExhibScanTime = 0.0
...
int function CalculateFullEnjoyment()
    if !bSLSO_Initialized
        SLSO_StartAnimating()
    endif

    ; 1. SLA Arousal (Кэш)
    int slaActorArousal = 0
    if iCfg_SLA_Mode == 2 && slaArousal
        slaActorArousal = ActorRef.GetFactionRank(slaArousal)
        if slaActorArousal < 0
            slaActorArousal = 0
        endif
    elseif iCfg_SLA_Mode == 3 && slaArousal
        slaActorArousalMod = (ActorRef.GetFactionRank(slaArousal) as float) * 0.02
        if slaActorArousalMod < 1.0
             slaActorArousalMod = 1.0
        endif
    endif

    ; 2. Exhibitionist (Используем переменную, обновляемую в OnUpdate, а не сканируем здесь)
    if !IsCreature && iCfg_Exhib_Mode > 0
        ; ... (код расчета ExhibitionistMod на основе slaExhibitionistNPCCount) ...
    endif
    ...
    if EstrusForcedEnjoymentMods
        ActorFullEnjoyment = (SLSO_FullEnjoyment * fCfg_Estrus_Mod) as int
    else
...
function BonusEnjoyment(actor Ref = none, int fixedvalue = 0)
	...
	if self.GetState() == "Animating"
		if Ref == none || Ref == ActorRef
			...
			endIf
			int schlongEnjoyment = 0
			;Add enjoyment to self based on penetrators schlongs sizes
			if Thread.ActorCount >= 2  && JsonUtil.GetIntValue(File, "slso_schlong_size_enjoyment_bonus") > 0 && ((Animation.HasTag("Vaginal") || Animation.HasTag("Anal")) || (JsonUtil.GetIntValue(File, "slso_schlong_size_oral_bonus") && Animation.HasTag("Blowjob"))) && Thread.Positions.Find(Ref) == 0
				;Multiple schlonging
				if Thread.ActorCount > 2 && ((Animation.HasTag("Vaginal") && Animation.HasTag("Anal")) || (JsonUtil.GetIntValue(File, "slso_schlong_size_oral_bonus") && Animation.HasTag("Blowjob") && (Animation.HasTag("Anal") || Animation.HasTag("Vaginal"))))
					int i = Thread.ActorCount
					While i > 0
						i -= 1
						if Thread.ActorAlias[i].GetRef() as Actor != ActorRef
							schlongEnjoyment += Math.Floor(Thread.ActorAlias[i].GetSchlongPercentEfficiency() * JsonUtil.GetIntValue(File, "slso_schlong_size_enjoyment_bonus"))
						endif
					endWhile
					Log(Ref.GetDisplayName() + " Enjoy " + schlongEnjoyment + " from multiple schlongs.")
					BonusEnjoyment += schlongEnjoyment
				;Singular schlonging
				elseif (SosApi.GetSize(Thread.ActorAlias[1].GetRef() as Actor) > 0 || (Thread.ActorAlias[1].IsCreature() && JsonUtil.GetIntValue(File, "slso_creature_size_bonus"))) && JsonUtil.GetIntValue(File, "slso_schlong_size_enjoyment_bonus") > 0
					Log(Ref.GetDisplayName() + " Enjoy " +  Math.Floor(Thread.ActorAlias[1].GetSchlongPercentEfficiency() * JsonUtil.GetIntValue(File, "slso_schlong_size_enjoyment_bonus")) + " from "+ (Thread.ActorAlias[1].GetRef() as Actor).GetDisplayName() +"s schlong.")
					BonusEnjoyment += Math.Floor(Thread.ActorAlias[1].GetSchlongPercentEfficiency() * JsonUtil.GetIntValue(File, "slso_schlong_size_enjoyment_bonus"))
				endif
			endif
			;Penetrator Enjoyment modifier
			if (Animation.HasTag("Anal") || Animation.HasTag("Vaginal") || Animation.HasTag("Blowjob")) && SosApi.GetSize(ActorRef) > 0 && Thread.ActorAlias[0].GetRef() != ActorRef
				BonusEnjoyment = Math.ceiling(BonusEnjoyment * Thread.ActorAlias[0].GetBonusPenetratorEnjoyment())
			endif
			;Self Enjoyment Modifier
			BonusEnjoyment = Math.ceiling(BonusEnjoyment * GetBonusSelfEnjoyment())
...
;Schlong efficiency calc
float function GetSchlongPercentEfficiency()
	return SchlongPercentEfficiency
endfunction

function SetSchlongPercentEfficiency()
	String File = "/SLSO/Config.json"
	String CreatureFile = "/SLSO/CreatureSize.json"
	float eff = 0
	Actor akActor
	if ActorRef != none
		 akActor = ActorRef
	endif
	if IsCreature && JsonUtil.GetIntValue(File, "slso_creature_size_bonus")
		if JsonUtil.GetFloatValue(CreatureFile, ActorRaceKey)
			eff = JsonUtil.GetFloatValue(CreatureFile, ActorRaceKey)
			Log("Found " + ActorRaceKey + " Schlong size as " + JsonUtil.GetFloatValue(CreatureFile, ActorRaceKey))
		else
			int SizeChoice = SkyMessage.Show("Choose a creature schlong size for " + AkActor.GetDisplayName() + " \n (For reference, Big is human schlong's maximum.)", "Small", "Medium", "Big", "Very Big","Huge","Massive", getIndex = true) as int
			if SizeChoice == 0
				JsonUtil.SetFloatValue(CreatureFile, ActorRaceKey,0.01)
				eff = 0.1
				Log("Size small, efficiency: " + eff)
			elseif SizeChoice == 1
				JsonUtil.SetFloatValue(CreatureFile, ActorRaceKey,0.5)
				eff = 0.5
				Log("Size medium, efficiency: " + eff)
			elseif SizeChoice == 2
				JsonUtil.SetFloatValue(CreatureFile, ActorRaceKey,1.0)
				eff = 1.0
				Log("Size big, efficiency: " + eff)
			elseif SizeChoice == 3
				JsonUtil.SetFloatValue(CreatureFile, ActorRaceKey,1.6)
				eff = 1.6
				Log("Size very big, efficiency: " + eff)
			elseif SizeChoice == 4
				JsonUtil.SetFloatValue(CreatureFile, ActorRaceKey,2.5)
				eff = 2.5
				Log("Size huge, efficiency: " + eff)
			elseif SizeChoice == 5
				JsonUtil.SetFloatValue(CreatureFile, ActorRaceKey,3.5)
				eff = 3.5
				Log("Size massive, efficiency: " + eff)
			endif
		endif
	elseif	(StorageUtil.GetFloatValue(akActor,"SchlongMult"))
		eff = StorageUtil.GetFloatValue(akActor,"SchlongMult")
	elseif SosApi.GetSize(akActor as Actor) > 0 && ((SosApi.GetSize(akActor as Actor) - JsonUtil.GetIntValue(File, "slso_schlong_size_medium")) != 0) && ((SosApi.GetMaxSchlongSize() - JsonUtil.GetIntValue(File, "slso_schlong_size_medium")) != 0)\
	&& (JsonUtil.GetFloatValue(File, "slso_schlong_size_curve") != 0)
		if (SosApi.GetSize(akActor as Actor) - JsonUtil.GetIntValue(File, "slso_schlong_size_medium")) > 0
			eff = ((Math.pow((SosApi.GetSize(akActor as Actor) - JsonUtil.GetIntValue(File, "slso_schlong_size_medium")),JsonUtil.GetFloatValue(File, "slso_schlong_size_curve"))) / (Math.pow(SosApi.GetMaxSchlongSize() - JsonUtil.GetIntValue(File, "slso_schlong_size_medium"),JsonUtil.GetFloatValue(File, "slso_schlong_size_curve"))))
		endif
	endif
	Log("Schlong efficiency of " + akActor.GetDisplayName() + " is "+ eff )	
	SchlongPercentEfficiency = eff
endfunction

;Other enjoyment modifier

float function GetBonusSelfEnjoyment()
	if (StorageUtil.GetFloatValue(ActorRef,"BonusSelfEnjoyment"))
		return (StorageUtil.GetFloatValue(ActorRef,"BonusSelfEnjoyment"))
	else
		return 1.0
	endif
endfunction

function SetBonusSelfEnjoyment(float eff)
	StorageUtil.SetFloatValue(ActorRef,"BonusSelfEnjoyment",eff)
endfunction

float function GetBonusPenetratorEnjoyment()
	if (StorageUtil.GetFloatValue(ActorRef,"BonusPenetratorEnjoyment"))
		return (StorageUtil.GetFloatValue(ActorRef,"BonusPenetratorEnjoyment"))
	else
		return 1.0
	endif
endfunction

function SetBonusPenetratorEnjoyment(float eff)
	StorageUtil.SetFloatValue(ActorRef,"BonusPenetratorEnjoyment",eff)
endfunction
...
function SetMentallyBroken(bool b)
	IsMentallyBroken = b
endFunction

bool Function GetMentallyBrokenState()
	if !ActorRef
		IsMentallyBroken = false
	endIf
	return IsMentallyBroken
endFunction

function SetResisting(bool b)
	IsResisting = b
endFunction

bool function GetResisting()
	return IsResisting 
endFunction

function AddEdgeStack(int i = 0)
	if i > 0
		EdgeStack += i
	endif
endfunction

function LoseEdgeStack(int i = 0)
	if i > 0 && EdgeStack > 0
		EdgeStack -= i
	else
		EdgeStack = 0
...
int function GetEdgeStack()
	return EdgeStack
endfunction

function SetLastMentalDamage(int i = 0)
	LastMentalDamage = i
endfunction

int function GetLastMentalDamage()
	return LastMentalDamage
endfunction

function SetLastMentalDamageTime(Float time = 0.0)
	LastMentalDamageTime = time
endfunction

Float function GetLastMentalDamageTime()
	return LastMentalDamageTime
endfunction

function AddTemperMult(Float f = 0.0)
	String File = "/SLSO/Config.json"
	if f > 0 && TemperMult < JsonUtil.GetFloatValue(File, "condition_maximum_aggressor_temper")
		TemperMult += f
		TemperMult = PapyrusUtil.ClampFloat(TemperMult,(JsonUtil.GetFloatValue(File, "condition_minimum_aggressor_temper") as Float) , (JsonUtil.GetFloatValue(File, "condition_maximum_aggressor_temper") as Float))
	endif
endfunction

function LoseTemperMult(Float f = 0.0)
	String File = "/SLSO/Config.json"
	if f > 0 && TemperMult > JsonUtil.GetFloatValue(File, "condition_minimum_aggressor_temper")
		TemperMult -= f
		TemperMult = PapyrusUtil.ClampFloat(TemperMult, (JsonUtil.GetFloatValue(File, "condition_minimum_aggressor_temper") as Float) , (JsonUtil.GetFloatValue(File, "condition_maximum_aggressor_temper") as Float))
	endif
endfunction

function ResetTemperMult()
		TemperMult = 1
endfunction

Float function GetTemperMult()
	return TemperMult
endfunction
...
int function SLSO_DoOrgasm_Conditions(bool Forced)
    ...
    ; ... (проверки LeadIn, Player и т.д. через кэш) ...
	String File = "/SLSO/Config.json"
	...
	if !Animation.HasTag("Estrus")
		if IsVictim
			; MIND BREAK CHECK (Мана < 10%)
			bool isBroken = ActorRef.GetActorValuePercentage("Magicka") < 0.10
			
			if iCfg_Victim_Orgasm_Mode == 0 ; No
				return -4
			elseif iCfg_Victim_Orgasm_Mode == 2 ; Lewd check
				if (OwnSkills[Stats.kLewd]*10) < Utility.RandomInt(0, 100)
					return -5
				endIf
			elseif iCfg_Victim_Orgasm_Mode == 3 ; Mind Broken Only
				if !isBroken
					return -13 ; Blocked: Not Broken
				endif
			elseif iCfg_Victim_Orgasm_Mode == 4 ; Mind Broken OR Lewd
				if !isBroken && (OwnSkills[Stats.kLewd]*10) < Utility.RandomInt(0, 100)
					return -14 ; Blocked: Not Broken AND Not Lewd
				endif
			endIf
		endIf
		if !IsAggressor
...
function SLSO_DoOrgasm_SexLabOrgasmSeparate()
	if SeparateOrgasms
        ; Restored Logic
        ActorRef.DamageActorValue("Magicka",(ActorRef.GetBaseActorValue("Magicka")*0.01)*GetEdgeStack())
        log("consumed "+ GetEdgeStack() + " edge stack")
        LoseEdgeStack()
...
function SLSO_StartAnimating()
    ...
    ; КЭШИРОВАНИЕ (Один раз за анимацию)
    iCfg_SLA_Mode = JsonUtil.GetIntValue(File, "sl_sla_arousal")
    iCfg_Exhib_Mode = JsonUtil.GetIntValue(File, "sl_exhibitionist")
    iCfg_Masturb_Mode = JsonUtil.GetIntValue(File, "sl_masturbation")
    iCfg_Victim_Orgasm_Mode = JsonUtil.GetIntValue(File, "condition_victim_orgasm")
    
    iCfg_Schlong_Bonus = JsonUtil.GetIntValue(File, "slso_schlong_size_enjoyment_bonus")
    iCfg_Schlong_Med = JsonUtil.GetIntValue(File, "slso_schlong_size_medium", 6)
    fCfg_Schlong_Curve = JsonUtil.GetFloatValue(File, "slso_schlong_size_curve", 1.0)
    bCfg_Creature_Bonus = JsonUtil.GetIntValue(File, "slso_creature_size_bonus") == 1
    bCfg_Schlong_Oral = JsonUtil.GetIntValue(File, "slso_schlong_size_oral_bonus") == 1
    fCfg_Estrus_Mod = JsonUtil.GetFloatValue(File, "sl_estrusforcedenjoyment")
    
    bSLSO_Initialized = true
...
;Estrus, force enjoyment modifiers to 1+
	...
	if EstrusAnim && fCfg_Estrus_Mod > 0
		EstrusForcedEnjoymentMods = true
	endif
	
;apply masturbation modifier 
	MasturbationMod = 1
	if Thread.ActorCount == 1 && iCfg_Masturb_Mode == 1
...
;		endIf
    ...
    ; Запускаем таймер для тяжелой проверки Эксибиционизма
    if iCfg_Exhib_Mode > 0
        RegisterForSingleUpdate(1.0) 
    endif
endFunction
