...
; --- SLSO CACHED VARIABLES (Optimization) ---
bool bCached_GameEnabled
bool bCached_SeparateOrgasms
bool bCached_Cond_Consensual
bool bCached_Cond_Aggressor
bool bCached_Cond_PlayerAggro
bool bCached_Cond_DDBelt
bool bCached_Cond_ChangeAnim
bool bCached_HybridProgression
int  iCached_HybridDelay
int  iCached_MinOrgasm
int  iCached_MaxOrgasm
float fCurrentStageStartTime ; Таймер для гибридной прогрессии
int iLastCheckedStage
; --------------------------------------------
...
function StartupDone()
    if Config.HasSLSO
        ...
        ; 1. ЧИТАЕМ НАСТРОЙКИ ОДИН РАЗ (Оптимизация)
        bCached_GameEnabled = JsonUtil.GetIntValue(File, "game_enabled") == 1
        bCached_SeparateOrgasms = Config.SeparateOrgasms
        bCached_Cond_Consensual = JsonUtil.GetIntValue(File, "condition_consensual_orgasm") == 1
        bCached_Cond_Aggressor = JsonUtil.GetIntValue(File, "condition_aggressor_orgasm") == 1
        bCached_Cond_PlayerAggro = JsonUtil.GetIntValue(File, "condition_player_aggressor_orgasm") == 1
        bCached_Cond_DDBelt = JsonUtil.GetIntValue(File, "condition_ddbelt_orgasm") == 1
        bCached_Cond_ChangeAnim = JsonUtil.GetIntValue(File, "condition_aggressor_change_animation") == 1
        
        ; Гибридная прогрессия
        bCached_HybridProgression = JsonUtil.GetIntValue(File, "game_hybrid_progression") == 1
        iCached_HybridDelay = JsonUtil.GetIntValue(File, "game_hybrid_progression_delay")
        
        if bCached_GameEnabled
            iCached_MinOrgasm = JsonUtil.GetIntValue(File, "condition_minimum_aggressor_orgasm")
            iCached_MaxOrgasm = JsonUtil.GetIntValue(File, "condition_maximum_aggressor_orgasm")
        else
            iCached_MinOrgasm = 1
            iCached_MaxOrgasm = 1
        endif

        SLSO_condition_minimum_aggressor_orgasm = iCached_MinOrgasm
        SLSO_condition_maximum_aggressor_orgasm = iCached_MaxOrgasm
        
        SLSO_shouldrun_SLSO_Animating_GoToStage = bCached_SeparateOrgasms && HasPlayer
        
        if (GetVictim() == none) 
            SLSO_shouldrun_SLSO_Animating_GoToStage = SLSO_shouldrun_SLSO_Animating_GoToStage && bCached_Cond_Consensual
        else
            SLSO_shouldrun_SLSO_Animating_GoToStage = SLSO_shouldrun_SLSO_Animating_GoToStage && (bCached_Cond_Aggressor || bCached_Cond_PlayerAggro)
        endif
        
        ; Сброс таймеров прогрессии
        fCurrentStageStartTime = 0.0
        iLastCheckedStage = 0
    endIf
    RegisterForSingleUpdate(0.1)
endFunction
...
state Animating
	...
	event OnUpdate()
		...
		; --- SLSO HYBRID PROGRESSION (ИСПРАВЛЕНО) ---
		if bCached_HybridProgression && !LeadIn && !TimedStage && Stage < StageCount
			; Find Leader (usually last actor/aggressor)
			sslActorAlias LeadActor = ActorAlias[ActorCount - 1]
			if LeadActor
				; Check if leader enjoyment exceeds current stage progress requirement
				if LeadActor.GetFullEnjoyment() > (Stage * (100 / StageCount))
					; Check delay using START time, not END time
					if (RealTime[0] - fCurrentStageStartTime) > iCached_HybridDelay
						GoToStage(Stage + 1)
						return
					endif
				endif
...
function SetAnimation(int aid = -1)
	...
	IsType[1]  = (Females + FemaleCreatures + Futas[0] + Futas[1] + Futas[2] + Futas[3]) > 0 && (Tags.Find("Vaginal") != -1 || Tags.Find("Pussy") != -1)
	IsType[2]  = Tags.Find("Anal")   != -1 || ((Females + FemaleCreatures + Futas[0] + Futas[1] + Futas[2] + Futas[3]) == 0 && Tags.Find("Vaginal") != -1)
        ...
        if SLSO_shouldrun_SLSO_Animating_GoToStage
            ...
            while i > 0
                ...
                if ActorAlias[i].GetRef() != none
                    bool isPlayer = (ActorAlias[i].GetRef() == GetPlayer())
                    ; Используем кэш
                    bool checkCondition = (isPlayer && bCached_Cond_PlayerAggro) || (!isPlayer && bCached_Cond_Aggressor)

                    if ActorAlias[i].IsAggressor() && checkCondition
                        if ((ActorAlias[i].IsCreature() && bCached_GameEnabled) || !ActorAlias[i].IsCreature())
                            if ActorAlias[i].GetOrgasmCount() < SLSO_condition_minimum_aggressor_orgasm && ActorAlias[i].IsOrgasmAllowed()
                                Bool Belted = false
                                
                                ; Проверка пояса через кэш
                                if !bCached_Cond_DDBelt
                                    Keyword zadDeviousBelt = Game.GetFormFromFile(0x3330, "Devious Devices - Assets.esm") As Keyword
                                    if zadDeviousBelt && (ActorAlias[i].GetRef() as Actor).WornHasKeyword(zadDeviousBelt)
                                        Belted = true
                                        i = 0
                                        Log("Lead is DD belted, ending animation")
                                    EndIf
                                EndIf
                                ...
                                if Belted == false
                                    ...
                                    if !isPlayer && bCached_Cond_ChangeAnim
                                        ToStage = -1
                                    endif
                                    
                                    i = 0
                                    Log("Lead is not satisfied, looping to stage " + ToStage)
                                endif
                            endIf
                        endIf
                    endIf
...
; ------------------------------------------------------- ;
; --- SLSO COOLDOWN LOGIC (Required for GameScript)   --- ;
; ------------------------------------------------------- ;

float fLastStageSkipTime = 0.0

Function SetLastStageSkip(float val)
    fLastStageSkipTime = val
EndFunction

Float Function GetLastStageSkip()
    return fLastStageSkipTime
EndFunction