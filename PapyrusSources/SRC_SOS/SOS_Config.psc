...
GlobalVariable Property SOS_DistributeAddons Auto
...
bool property bDisplayActionInConsole auto Hidden
bool property bDistributeAddons
	bool function get()
		bool output = SOS_DistributeAddons.GetValueInt() as bool
		If output && !SOS_Quest.SOS_SetupNPCSpell ; Revert the "SoS Uncloaked.esp"
			SOS_Quest.SOS_SetupNPCSpell = Game.GetFormFromFile(0x12e7, "Schlongs of Skyrim.esp") as Spell
		EndIf
		return output
	endFunction
	function set(bool value)
		If Value && !SOS_Quest.SOS_SetupNPCSpell ; Revert the "SoS Uncloaked.esp"
			SOS_Quest.SOS_SetupNPCSpell = Game.GetFormFromFile(0x12e7, "Schlongs of Skyrim.esp") as Spell
		EndIf
		SOS_DistributeAddons.SetValueInt(value as int)
	endFunction
EndProperty
...
Event OnConfigInit()
	...
	If !SOS_DistributeAddons
		SOS_DistributeAddons = Game.GetFormFromFile(0x228D5, "Schlongs of Skyrim.esp") as GlobalVariable
	EndIf
	InitPages()
EndEvent
...
Event OnPageReset(String page)
	...
	if(page == "$General Settings")
		...
		oidDisplayActionInConsole = AddToggleOption("$Console Output Header", bDisplayActionInConsole)
		...
		oidDistribute = AddToggleOption("$Distribute Addons", bDistributeAddons)
	...
	If page == "$SexLab Gender"
		int count = SOS_Data.CountAddons()
		oidForceMale = Utility.CreateIntArray(count + 1)
		oidForceFemale = Utility.CreateIntArray(count + 1)
		int i = 0
		
		SetCursorFillMode(TOP_TO_BOTTOM)
		
		AddHeaderOption("Force SexLab Gender to male for : ")
		while(i < count && i < 127)
			SOS_AddonQuest_Script sosAddon = SOS_Data.GetAddon(i)
			if sosAddon
				if sosAddon.bForceFemale || !SOS_Quest.SexLabGenderFaction
					oidForceMale[i] = AddToggleOption(sosAddon.GetAddonName(), sosAddon.bForceMale, OPTION_FLAG_DISABLED) 
				else
					oidForceMale[i] = AddToggleOption(sosAddon.GetAddonName(), sosAddon.bForceMale) 
				endif
			EndIf
			i += 1
		endwhile
		...
		AddHeaderOption("Force SexLab Gender to female for : ")
		while(i < count && i < 127)
			SOS_AddonQuest_Script sosAddon = SOS_Data.GetAddon(i)
			If sosAddon
				if sosAddon.bForceMale || !SOS_Quest.SexLabGenderFaction
					oidForceFemale[i] = AddToggleOption(sosAddon.GetAddonName(), sosAddon.bForceFemale, OPTION_FLAG_DISABLED) 
				else
					oidForceFemale[i] = AddToggleOption(sosAddon.GetAddonName(), sosAddon.bForceFemale) 
				endif
			EndIf
			i += 1
		endwhile
...
Function FillActorSettings(Actor akActor, Bool isPlayer)
	...
	If akArmor
		...
		isRevealing = SOS_Quest.IsRevealing(akArmor)

		If isRevealing && SOS_Quest.HasRevealingKeyword(akArmor)
			; built-in revealing armors can't be modified
			optionName += " (built-in)"
			if Math.LogicalAnd(akArmor.GetSlotMask(), 0x00400004) == 0x00400004
				akArmor.RemoveSlotFromMask(0x00400000)
			EndIf
		ElseIf !akActor.IsInFaction(SOS_Quest.SOS_SchlongifiedFaction)
			optionName += " (un-schlongified)"
		Else
			flags = OPTION_FLAG_NONE
		EndIf
	EndIf
...
Event OnOptionSelect(int option)
	...
	elseif option == oidImport
		...
		If result == 0
			...
			ForcePageReset()
		Else
			ShowMessage("Import failed\nEither the settings file is missing or it has invalid content (\"SKSE\\Plugins\\SOS.json\")", false)
		EndIf
	elseif option == oidExport
		bool ok = (Quest.GetQuest("SOS_Misc") as SOS_IO).ExportSettings()
		If ok
			ShowMessage("File \"SKSE\\Plugins\\SOS.json\" created", false)
		Else
	...
	elseif option == oidPlayerArmorRevealing
		Armor akArmor = Game.GetPlayer().GetWornForm(0x00000004) as Armor
		SOS_Quest.ToggleRevealingArmor(Game.GetPlayer())
		SetToggleOptionValue(option, SOS_Quest.IsRevealing(akArmor))
	elseif option == oidNPCArmorRevealing
		Armor akArmor = PlayerAliasScript.selectedActor.GetWornForm(0x00000004) as Armor
	...
	ElseIf option == oidNPCDebug
		...
		CloseMCM()
	elseif option == oidDisplayActionInConsole
		bDisplayActionInConsole = !bDisplayActionInConsole
		SetToggleOptionValue(oidDisplayActionInConsole, bDisplayActionInConsole)
	elseif option == oidDistribute
		bDistributeAddons = !bDistributeAddons
		SetToggleOptionValue(oidDistribute, bDistributeAddons)
	endif
	
	int i = 0
	int count = SOS_Data.CountAddons()
	while(i < count && i < 127)
		if(option == oidForceMale[i])
			SOS_AddonQuest_Script sosAddon = SOS_Data.GetAddon(i)
			If sosAddon
				if(sosAddon.bForceMale)
					sosAddon.bForceMale = false
					SetToggleOptionValue(oidForceMale[i], sosAddon.bForceMale)
					SetOptionFlags(oidForceFemale[i], OPTION_FLAG_NONE)
				else
					sosAddon.bForceMale = true
					sosAddon.bForceFemale = false
					SetToggleOptionValue(oidForceMale[i], sosAddon.bForceMale)
					SetOptionFlags(oidForceFemale[i], OPTION_FLAG_DISABLED)
				endif
			endif
		endif
		if(option == oidForceFemale[i])
			SOS_AddonQuest_Script sosAddon = SOS_Data.GetAddon(i)
			If sosAddon
				if(sosAddon.bForceFemale)
					sosAddon.bForceFemale = false
					SetToggleOptionValue(oidForceFemale[i], sosAddon.bForceFemale)
					SetOptionFlags(oidForceMale[i], OPTION_FLAG_NONE)
				else
					sosAddon.bForceFemale = true
					sosAddon.bForceMale = false
					SetToggleOptionValue(oidForceFemale[i], sosAddon.bForceFemale)
					SetOptionFlags(oidForceMale[i], OPTION_FLAG_DISABLED)
				endif
			endif
		endif
		i += 1
	...
	if(currentAddonIndex > -1)
		...
		if(raceIndex > -1 && IsAddonOptionRaceEnable(option))
			...
			if(changedRace)
				...
				SOS_Quest.InvalidateRaceCache(changedRace)
			endif
	...
	elseif(currentAddonIndex > -1)
		...
		elseif(raceIndex > -1)
			...
			elseif(IsAddonOptionRaceProbability(option))
				...
				SOS_Quest.InvalidateRaceCache(SOS_Data.GetCompatibleRace(addon, raceIndex))
...
Event OnOptionHighlight(int option)
	...
	elseif(option == oidDisplayActionInConsole)
		SetInfoText("$Console Output Option")
	elseif(option == oidDistribute)
		SetInfoText("$When Enabed, automatically distribute the addons bettwen the NPC's. Disable it if you are using one SPID '_DIST.ini' file for the SOS intead")
	elseif(option == oidNotificationsEnabled)
...
Function InitPages()
	...
	int pageCount = SOS_Data.CountAddons() + 3
	if( pageCount > 127)
		pageCount = 127
	endif
	...
	pages[2] = "$SexLab Gender"
	int i = 0
	int count = SOS_Data.CountAddons()
	while(i < count && i < 127)
		SOS_AddonQuest_Script sosAddon = SOS_Data.GetAddon(i)
		If sosAddon
			pages[i+3] = sosAddon.GetAddonName()
		Else
			pages[i+3] = "$missing_schlong"
		EndIf
		i += 1
	endwhile
