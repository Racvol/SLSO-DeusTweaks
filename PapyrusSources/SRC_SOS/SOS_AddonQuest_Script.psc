...
bool Property bForceMale = false Auto
bool Property bForceFemale = false Auto
...
Function RegisterAddon()
	...
	Debug.Trace("SOS: Addon " + addonName + " registered.")
	Debug.Notification("SOS Addon " + addonName + " loaded!") ; todo translate
	
	; You can't expect every SOS addon author to fill a new property in their plugin, so we have to retrieve SOS_SetupQuest this way.
	(Game.GetFormFromFile(0xD62, "Schlongs of Skyrim.esp") as SOS_SetupQuest_Script).AddonsRegistered = True
...
bool Function SelfCheck()
	...
	if(!(SOS_Addon_CompatibleRaces && SOS_Addon_EnabledRaces && SOS_Addon_RaceSizes && SOS_Addon_RaceProbabilities))
		Debug.Trace("SOS: Dirty addon '" + addonName + "' - settings collection references missing or invalid.")
		return false
	else
		int raceCount = SOS_Addon_CompatibleRaces.GetSize()
	...
	if(!(SOS_Addon_Faction && SOS_Addon_Genitals && SOS_Addon_Potion && SOS_Addon_Gender && SOS_Addon_Schlonged))
		Debug.Trace("SOS: Dirty addon '" + addonName + "' - asset references missing or invalid.")
		return false
	endif
	...
	if(!(SOS_Addon_Bone_GenBase && SOS_Addon_Bone_GenScrotum && SOS_Addon_Bone_Gen01 && SOS_Addon_Bone_Gen02 && SOS_Addon_Bone_Gen03 && SOS_Addon_Bone_Gen04 && SOS_Addon_Bone_Gen05 && SOS_Addon_Bone_Gen06))
		Debug.Trace("SOS: Dirty addon '" + addonName + "' - subshape global references missing or invalid.")
		return false
	endif
	
	; sos core esm references
	If !SOS_Addons
		Debug.Trace("SOS: Dirty addon '" + addonName + "' - core esm references missing or invalid.")
		Return False
	EndIf
...
Function UnregisterAddon()
	...
	Debug.Trace("SOS: Trying to remove everything added by the " + addonName + " addon...")
	...
	Debug.Trace("SOS: Unregistration of " + addonName + " addon hopefully complete.")
	Debug.Notification("SOS Addon " + addonName + " unregistered!") ; todo translate
EndFunction
