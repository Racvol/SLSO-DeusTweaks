...
function ApplyPhase(Actor ActorRef, int Phase, int Gender)
	...
	if Phase <= Phases[Gender]
		...
		if !akItem
			UnequipFaceItem(ActorRef)
		endIf
		float ExpressionTransitSpeed = SexLabUtil.GetConfig().ExpressionTransitSpeed 
		If ExpressionTransitSpeed < 0.1
			ApplyPresetFloats(ActorRef, GenderPhase(Phase, Gender))
		Else
			TransitPresetFloats(ActorRef, GetCurrentMFG(ActorRef), GenderPhase(Phase, Gender), ExpressionTransitSpeed, Math.Abs((Phase / Phases[Gender]) - 1.0)) 
		endIf
		
		If EquipFaceItem(ActorRef, akItem) && ActorRef == Game.GetPlayer() && (Game.GetCameraState() == 0 || Game.GetCameraState() == 3)
			ApplyPresetFloats(ActorRef, GenderPhase(Phase, Gender))
		endIf
	endIf
endFunction
...
;	endIf
	...
	if (GetExpression(ActorRef, true) as int != OpenMouthExpression || GetExpression(ActorRef, false) != value as float / 100.0)
		ActorRef.SetExpressionOverride(OpenMouthExpression, value)
	endIf
	; Set Phoneme
	...
	while i < Phonemes.length
		...
		if (GetPhoneme(ActorRef, i) != value as float / 100.0)
			if HasMFG
				MfgConsoleFunc.SetPhonemeModifier(ActorRef, 0, i, value)
			else
	...
	if PhonemeUpdated
		...
		if HasMFG
			MfgConsoleFunc.SetPhonemeModifier(ActorRef, 0, s, value) ; Oldrim
		else
...
;	return GetPhoneme(ActorRef, 1) >= MinMouthSize && (GetExpression(ActorRef, true) as Int == OpenMouthExpression && GetExpression(ActorRef, false) >= MinMouthSize)
	if GetExpression(ActorRef, true) as Int == OpenMouthExpression && GetExpression(ActorRef, false) >= (PapyrusUtil.ClampInt((MinMouthScale * (SexLabUtil.GetConfig().OpenMouthSize as float / 100.0)) as int, 0, 100) as float / 100.0)
		return true
	endIf

;	int MouthScale = (StorageUtil.GetFloatValue(ActorRef, "SexLab.MouthScale", 1.0) * Scale * 100) as Int
	float MouthScale = 0.0
	float TempPhoneme
	float[] Phonemes = SexLabUtil.GetConfig().GetOpenMouthPhonemes(isRealFemale)											 
	Int i = 0
	while i < Phonemes.length
		TempPhoneme = GetPhoneme(ActorRef, i)
		If MouthScale != 0.0 && TempPhoneme != 0.0
			MouthScale = Phonemes[i] - TempPhoneme
			float absMouthScale = math.abs(MouthScale)
			If absMouthScale < 0.2 || absMouthScale > 2.0
				return false ; The difference should stay into the valid range
			endIf
		EndIf
		If MouthScale != Phonemes[i] - TempPhoneme
			return false ; The values should match or at least have the same difference
		endIf
		i += 1
	endWhile
	return true
...
function ApplyPresetFloats(Actor ActorRef, float[] Preset) global 
	...
	If (GetExpression(ActorRef, true) != Preset[30] || GetExpression(ActorRef, false) != value as Float / 100.0) && !IsMouthOpen
		ActorRef.SetExpressionOverride(Preset[30] as int, value)
	endIf
	; Set Phoneme
	if IsMouthOpen
		...
		while p <= 15
			...
			if GetPhoneme(ActorRef, p) != value as float / 100.0
				if HasMFG
					MfgConsoleFunc.SetPhonemeModifier(ActorRef, 0, p, value) ; Oldrim
				else
...
bool function EquipFaceItem(Actor ActorRef, Form akItem) global
	if !ActorRef || !akItem
		return False
	endIf

	armor akArmor = akItem as Armor
	if akArmor && Math.LogicalAnd(akArmor.GetSlotMask(), 0x80000000) == 0x80000000 && (ActorRef.IsEquipped(akItem) || ActorRef.GetItemCount(akItem) > 0)
...
;/ 			if ActorRef == Game.GetPlayer() && (Game.GetCameraState() == 0 || Game.GetCameraState() == 3)
				ActorRef.UnequipItemSlot(61)
				ActorRef.EquipItem(akItem, false, true)
				NiOverride.UpdateModelWeight(ActorRef)
			;	ActorRef.UpdateWeight(0.0)
			endif
 /;			
			Utility.Wait(0.1)
...
bool function UnequipWornFaceItem(Actor ActorRef, sslBaseExpression oldExpression, sslBaseExpression newExpression = none) global
	...
	if oldExpression != newExpression
		...
		if akWorn && oldExpression.GetEquipments(Gender).Find(akWorn) != -1 ;&& (!newExpression || newExpression.GetEquipments(Gender).Find(akWorn) == -1)
			return UnequipFaceItem(ActorRef, akWorn)
		endIf
	endIf
	return False
endFunction

bool function UnequipFaceItem(Actor ActorRef, Form akItem = none) global
	if !ActorRef
		return False
	endIf
	...
	if akItem
		...
		if TempFaceItems && TempFaceItems.Find(akItem) != -1
			akWorn = akItem ; Remove only the supported face items
		endif
	else
		...
		if akWorn
			...
			if !TempFaceItems || TempFaceItems.Find(akWorn) == -1
				akWorn = None ; Remove only the supported face items
			endif
		endIf
	endif
	if akWorn
		If ActorRef.IsEquipped(akWorn)
;/ 			Form EmptyItem = Game.GetFormFromFile(0x8FC65, "SexLab.esm") ; SexLabFaceItemEmpty "Empty" [ARMO:0808FC65]
			If EmptyItem && (EmptyItem == akWorn || (EmptyItem.GetType() == 26 && ActorRef.GetItemCount(EmptyItem) > 0))
				...
				ActorRef.UnequipItem(EmptyItem, false, true)
			EndIf
 /;
