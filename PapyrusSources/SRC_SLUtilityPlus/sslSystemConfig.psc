...
float property ExpressionTransitSpeed auto hidden
...
bool function UnequipActorFaceItems(Actor ActorRef)
	return sslBaseExpression.UnequipFaceItem(ActorRef)
endFunction
...
;	endWhile
;	return Phonemes
	; This is the most native way of make a independent copy of an Array
	if isFemale
		return Utility.ResizeFloatArray(Utility.ResizeFloatArray(OpenMouthFemale, 17), 16)
	else
		return Utility.ResizeFloatArray(Utility.ResizeFloatArray(OpenMouthMale, 17), 16)
	endIf
...
bool function SetOpenMouthPhonemes(bool isFemale, float[] Phonemes)
	...
	ElseIf OpenMouthFemale.Length < 16
		OpenMouthFemale = Utility.ResizeFloatArray(OpenMouthFemale, 17)
	endIf
	if !OpenMouthMale
		OpenMouthMale = new float[17]
	ElseIf OpenMouthMale.Length < 16
		OpenMouthMale = Utility.ResizeFloatArray(OpenMouthMale, 17)
	endIf
	int i = 16
	while i > 0
		i -= 1
...
bool function SetOpenMouthExpression(bool isFemale, int value)
	if isFemale
		if !OpenMouthFemale
			OpenMouthFemale = new float[17]
		ElseIf OpenMouthFemale.Length < 17
			OpenMouthFemale = Utility.ResizeFloatArray(OpenMouthFemale, 17)
		endIf
		...
		if !OpenMouthMale
			OpenMouthMale = new float[17]
		ElseIf OpenMouthMale.Length < 17
			OpenMouthMale = Utility.ResizeFloatArray(OpenMouthMale, 17)
		endIf
...
;	return Offsets
	...
	; This is the most native way of make a independent copy of an Array
	return Utility.ResizeFloatArray(Utility.ResizeFloatArray(BedOffset, 5), 4)
endFunction
...
;			FurnitureOffsets = TableOffset
			return Utility.ResizeFloatArray(Utility.ResizeFloatArray(TableOffset, 5), 4)
		elseIf FurnitureTags[FurnitureType] == "BenchTable"
;			FurnitureOffsets = BenchTableOffset
			return Utility.ResizeFloatArray(Utility.ResizeFloatArray(BenchTableOffset, 5), 4)
		else
			; Chair, Throne and Bench have all the same higth and center
;			FurnitureOffsets = FurnitureOffset
			return Utility.ResizeFloatArray(Utility.ResizeFloatArray(FurnitureOffset, 5), 4)
		endIf
	endIf
...
function LoadStrapons()
	...
	if sslUtility.IsPluginInstalled("StrapOnbyaeonv1.1.esp")
		LoadStrapon("StrapOnbyaeonv1.1.esp", 0xD65)
	endIf
	...
	endIf
	if sslUtility.IsPluginInstalled("SOS Equipable Schlong.esp")
		LoadStrapon("SOS Equipable Schlong.esp", 0xD62)
	endIf
	ModEvent.Send(ModEvent.Create("SexLabLoadStrapons"))
endFunction

Armor function LoadStrapon(string esp, int id)
	if sslUtility.IsPluginInstalled(esp)
		Form Strapon = Game.GetFormFromFile(id, esp)
		if (Strapon as Armor) && Strapons.Find(Strapon) == -1
...
bool function CheckSystemPart(string CheckSystem)
	if CheckSystem == "Skyrim"
		return (StringUtil.SubString(Debug.GetVersionNumber(), 0, 3) as float) >= 1.6 ;SSE

	elseIf CheckSystem == "SKSE"
		return SKSE.GetScriptVersionRelease() >= 66 ;SSE

	elseIf CheckSystem == "SkyUI"
		return Quest.GetQuest("SKI_ConfigManagerInstance") != none

	elseIf CheckSystem == "SexLabUtil"
		return SexLabUtil.GetPluginVersion() >= 16401 ;SSE

	elseIf CheckSystem == "PapyrusUtil"
		return PapyrusUtil.GetVersion() >= 41 ;SSE
	...
	elseIf CheckSystem == "FNISSexLabCreature" && PlayerRef.Is3DLoaded() && Game.GetCameraState() > 3 && FNIS.VersionCompare(7, 0, 0, true) >= 0 ;SSE
...
bool function CheckSystem()
	...
	if !CheckSystemPart("Skyrim")
		CheckSkyrim.Show(1.6) ;SSE
		return false
	; Check SKSE install
	elseIf !CheckSystemPart("SKSE")
		CheckSKSE.Show(2.0102) ;SSE
		return false
...
function Reload()
	...
	HasSLSO = sslUtility.IsPluginInstalled("SLSO.esp")
	; - SexLab Aroused
	...
	HasUtilityPlus = sslUtility.IsPluginInstalled("SexLab UtilityPlus.esp") ;SSE
	; - Furnitures
