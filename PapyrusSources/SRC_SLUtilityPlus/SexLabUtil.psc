...
float function RealSecondsToGameHours(Float fSeconds) global
	...
	endIf
	float output =  (fSeconds / 86400.0) * fTimeScale
;	Log(output, "RealSecondsToGameHours("+fSeconds+")")
	return output
endFunction
...
function EnableFreeCamera(bool Enabling = true, float sucsm = 5.0) global
	...
	Log("- CameraState: "+CameraState, "EnableFreeCamera("+Enabling+", "+sucsm+")")
