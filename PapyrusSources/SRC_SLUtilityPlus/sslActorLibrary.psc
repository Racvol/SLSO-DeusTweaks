...
Form[] function StripSlots(Actor ActorRef, bool[] Strip, bool DoAnimate = false, bool AllowNudesuit = true)
	...
	;endIf
	; Get Nudesuit
	bool UseNudeSuit = AllowNudesuit && Strip[2] && ((Gender == 0 && Config.UseMaleNudeSuit) || (Gender == 1 && Config.UseFemaleNudeSuit))
	...
	if Strip.Length > 33 && Strip[33]
		...
		ElseIf IsPlayer && Config.DummyArrow
			...
			While i > 0
				...
				;	if ContinueStrip(ItemRef, Strip[34])
						Stripped[34] = ItemRef
						ActorRef.UnequipItemEX(ItemRef, 0, false)
				;	endIf
					i = 0
	...
	endIf
	; Strip Head
	ItemRef = ActorRef.GetWornForm(Armor.GetMaskForSlot(30))
	if ContinueStrip(ItemRef, Strip[0])
		ActorRef.UnequipItemEX(ItemRef, 0, false)
		Stripped[0] = ItemRef
	endIf
	ItemRef = ActorRef.GetWornForm(Armor.GetMaskForSlot(31))
	if ContinueStrip(ItemRef, Strip[1])
		ActorRef.UnequipItemEX(ItemRef, 0, false)
		Stripped[1] = ItemRef
	endIf
	; Strip armors
