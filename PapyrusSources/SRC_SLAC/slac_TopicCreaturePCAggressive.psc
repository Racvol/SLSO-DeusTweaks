...
Scriptname slac_TopicCreaturePCAggressive Extends TopicInfo Hidden

;BEGIN FRAGMENT Fragment_0
Function Fragment_0(ObjectReference akSpeakerRef)
Actor akSpeaker = akSpeakerRef as Actor
;BEGIN CODE
	; Player allows creature to use force
	...
	If !slacUtility.TestCreature(Creature, invited = True) || !slacUtility.TestVictim(PlayerRef,Creature, inviting = True)
		slacConfig.debugSLAC && slacUtility.Log("PC Creature Dialogue Aggressive test failure for " + CreatureNameRef)
	...
	If slacUtility.StartCreatureSex(PlayerRef, Creature, nonConsensual = False, requireTags = "Aggressive")
		slacNotify.Show("SexStart", PlayerRef, Creature, Consensual = True, Group = False)
		slacConfig.debugSLAC && slacUtility.Log("PC Creature Dialogue Aggressive start sex with " + CreatureNameRef)
	Else
		slacUtility.UnstripActor(PlayerRef)
		slacUtility.StopInviteAnimation(PlayerRef,Creature)
		slacConfig.debugSLAC && slacUtility.Log("PC Creature Dialogue Aggressive start sex failure for " + CreatureNameRef)
		slacUtility.slacData.SetSignalBool(PlayerRef, "RoughConsensual", True)
