...
{SLSO version - Uses OSLAroused API with SLSO-specific functionality}

; sla properties - kept for compatibility with SLSO
...
; Note to modders : do not call IsActorNaked() function it is heavy, use OSLArousedNative.IsActorNaked() instead
				...
				If (victim != None && victim == targetactorList[i] && JsonUtil.GetIntValue("/SLSO/Config", "condition_victim_arousal") != 1)
					...
					ElseIf (JsonUtil.GetIntValue("/SLSO/Config", "condition_victim_arousal") == 3 || JsonUtil.GetIntValue(File, "condition_victim_arousal") == 4)
						if targetactorList[i] == victim && (controller.ActorAlias(targetactorList[i]) as sslActorAlias).GetMentallyBrokenState()
							exposuremult = 1
							Sexlab.Log("victim mentally broken, sla exposure enabled")
					...
					ElseIf JsonUtil.GetIntValue(File, "condition_victim_orgasm_aggressor_broken") && aggressorBroken == controller.ActorCount - 1
						exposuremult = 1
						Sexlab.Log("aggressor mentally broken, sla exposure enabled")
...
; Storage cleaning is no longer needed with OSLAroused - it handles its own cleanup
function CleanActorStorage()
	debug.notification("Actor Storage Cleaning not needed with OSLAroused")
endFunction
...
function ClearFromActorStorage(Form FormRef)
	; No longer needed - OSLAroused handles cleanup
endFunction
