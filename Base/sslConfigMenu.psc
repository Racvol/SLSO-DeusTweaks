...
event OnGameReload()
	string log = "SexLab MCM Loaded CurrentVerison: "+CurrentVersion+" / "+SexLabUtil.GetVersion()
	Debug.Trace(log)
...
event OnPageReset(string page)
	...
	elseif page != ""
		...
		; Toggle Furniture Tags Settings
		elseIf page == "$SSL_FurnitureSettings"
			FurnitureSettings()
...
event OnConfigOpen()
	...
	if !SystemAlias.IsInstalled
		...
		if Config.HasUtilityPlus
			...
			Pages[2]  = "$SSL_FurnitureSettings"
			Pages[3]  = "$SSL_SoundSettings"
			Pages[4]  = "$SSL_PlayerHotkeys"
			Pages[5]  = "$SSL_TimersStripping"
	...
	endIf
	if BedOpt.Length != 4 || BedOpt.Find("") != -1
		BedOpt = new string[4]
		BedOpt[0] = "$SSL_Never"
		BedOpt[1] = "$SSL_Always"
		BedOpt[2] = "$SSL_NotVictim"
		BedOpt[3] = "Use without Ask"
	endIf
	...
	if Biped.Length != 34 || Biped.Find("") != -1
		Biped = new string[34]
		Biped[0]  = "$SSL_Head"
		Biped[1]  = "$SSL_Hair"
		Biped[2]  = "$SSL_Torso"
...
event OnConfigClose()
	...
	if Config.AllowFurniture && Config.ActiveFurnitureTags.Length <= 0
		Config.AllowFurniture = false
	endIf
	if AutoRealign
		AutoRealign = false
		if ThreadControlled
...
string[] function PaginationMenu(string asBeforePages = "", string asAfterPages = "", int aiCurrentPage) ;agumenst names changed for compatibility reasons (parent property with same name)
	string[] Output
;	Log("LastPage:"+LastPage+"; PerPage:"+PerPage+"; CurrentPage:"+CurrentPage+"; Slots:"+AnimationSlots.Slotted)
	if asBeforePages != ""
		Output = PapyrusUtil.PushString(Output, asBeforePages)
	endIf
	if aiCurrentPage < LastPage
	...
	endIf
	if asAfterPages != ""
		Output = PapyrusUtil.PushString(Output, asAfterPages)
	endIf
	return Output
endfunction
...
event OnHighlightST()
	...
	; Furniture Tag Toggle
	elseIf Options[0] == "FurnitureTag"
		SetInfoText("$SSL_ToggleFurnitureTags{"+Config.FurnitureTags[(Options[1] as int)]+"}")
	...
	; Expression
	elseIf Options[0] == "UseFaceItems"
		SetInfoText("$SSL_ToggleUseFaceItems")

	elseIf Options[0] == "ExpressionBeastOnly"
		SetInfoText("$SSL_ToggleExpressionBeastOnly")

	elseIf Options[0] == "ExpressionNoBeast"
		SetInfoText("$SSL_ToggleExpressionNoBeast")

	elseIf Options[0] == "ImportFaceItem"
		SetInfoText("$SSL_InfoImportFaceItem{"+GetItemName(PlayerRef.GetWornForm(0x80000000))+"}")

	elseIf Options[0] == "LipFixedFemale"
		SetInfoText("$SSL_InfoLipFixed")

	elseIf Options[0] == "LipFixedMale"
		SetInfoText("$SSL_InfoLipFixed")

	elseIf Options[0] == "FaceEquipFemale"
		SetInfoText("$SSL_InfoFaceEquip")

	elseIf Options[0] == "FaceEquipMale"
		SetInfoText("$SSL_InfoFaceEquip")
	...
	elseIf Options[0] == "MouthScalePlayer"
		SetInfoText("$SSL_InfoMouthScalePlayer")

	elseIf Options[0] == "MouthScaleTarget"
		SetInfoText("$SSL_InfoMouthScaleTarget")

	elseIf Options[0] == "LipsSoundTime"
		SetInfoText("$SSL_InfoLipsSoundTime")

	; Allow Furniture
	elseIf Options[0] == "AllowFurniture"
		SetInfoText("$SSL_InfoAllowFurniture")

	; Allow Furniture
	elseIf Options[0] == "UseAdvancedFurnitureSystem"
		SetInfoText("$SSL_InfoUseAdvancedFurnitureSystem")

	; NPCs Use Furniture
	elseIf Options[0] == "NPCFurniture"
		SetInfoText("$SSL_InfoNPCFurniture")

	; Ask Furniture
	elseIf Options[0] == "AskFurniture"
		SetInfoText("$SSL_InfoAskFurniture")

	; Restrict Fetish Tags
	elseIf Options[0] == "RestrictFetishTags"
		SetInfoText("$SSL_InfoRestrictFetishTags")

	; ZaD Filters
	elseIf Options[0] == "ManageZadFilter"
		SetInfoText("$SSL_InfoManageZadFilter")

	; ZaZ Filters
	elseIf Options[0] == "ManageZazFilter"
		SetInfoText("$SSL_InfoManageZazFilter")

	; Use Intimacy Circle
	elseIf Options[0] == "UseIntimacyCircle"
		SetInfoText("$SSL_InfoUseIntimacyCircle")
	...
	elseIf Options[0] == "InstallError"
		SetInfoText("$SSL_WarnFileIntegrityError")

	elseIf Options[0] == "FNISWarning"
		SetInfoText("$SSL_WarnCheckFNIS")
	endIf
endEvent
...
event OnSliderOpenST()
	...
	elseIf Options[0] == "MouthScalePlayer"
		SetSliderDialogStartValue((StorageUtil.GetFloatValue(PlayerRef, "SexLab.MouthScale", 1.0) * 100) as Int)
		SetSliderDialogRange(20, 200)
		SetSliderDialogInterval(5)
		SetSliderDialogDefaultValue(100)

	elseIf Options[0] == "MouthScaleTarget"
		SetSliderDialogStartValue((StorageUtil.GetFloatValue(TargetRef, "SexLab.MouthScale", 1.0) * 100) as Int)
		SetSliderDialogRange(20, 200)
		SetSliderDialogInterval(5)
		SetSliderDialogDefaultValue(100)
...
event OnSliderAcceptST(float value)
	...
	elseIf Options[0] == "AnimationOffset"
		...
		if Options[1] == "Furniture"
			Animation.SetFurnitureOffsets(AnimOffsets[0], AnimOffsets[1], AnimOffsets[2], AnimOffsets[3])
			Animation.ExportOffsets("FurnitureOffset")
		else
	...
	elseIf Options[0] == "MouthScalePlayer"
		If value == 100
			StorageUtil.UnsetFloatValue(PlayerRef, "SexLab.MouthScale")
			StorageUtil.FormListRemove(none, "SexLab.MouthScaleActors", PlayerRef, True)
		Else
			StorageUtil.SetFloatValue(PlayerRef, "SexLab.MouthScale", value / 100.0)
			StorageUtil.FormListAdd(none, "SexLab.MouthScaleActors", PlayerRef, False)
		EndIf
		SetSliderOptionValueST((StorageUtil.GetFloatValue(PlayerRef, "SexLab.MouthScale", 1.0) * 100) as Int, "{0}")

	elseIf Options[0] == "MouthScaleTarget"
		If value == 100
			StorageUtil.UnsetFloatValue(TargetRef, "SexLab.MouthScale")
			StorageUtil.FormListRemove(none, "SexLab.MouthScaleActors", TargetRef, True)
		Else
			StorageUtil.SetFloatValue(TargetRef, "SexLab.MouthScale", value / 100.0)
			StorageUtil.FormListAdd(none, "SexLab.MouthScaleActors", TargetRef, False)
		EndIf
		SetSliderOptionValueST((StorageUtil.GetFloatValue(TargetRef, "SexLab.MouthScale", 1.0) * 100) as Int, "{0}")
...
event OnMenuOpenST()
	...
	; Expression
	if Options[0] == "FaceEquipFemale"
		form[] FaceItemsList = Config.GetFaceItems()
		string[] FaceItemsListNames = Utility.CreateStringArray(FaceItemsList.Length + 1)
		int i = FaceItemsListNames.Length
	;	Log("FaceEquipFemale "+FaceItemsList.Length+"; "+i)
		FaceItemsListNames[0] = "( NONE )"
		while i > 1
			i -= 1
			FaceItemsListNames[i] = GetItemName(FaceItemsList[i - 1])
		endWhile
		Form EquipmentPhase = Expression.GetEquipmentPhase(Phase, Female)
		if EquipmentPhase
			SetMenuDialogStartIndex(FaceItemsList.Find(EquipmentPhase) + 1)
		else
			SetMenuDialogStartIndex(0)
		endIf
		SetMenuDialogDefaultIndex(0)
		SetMenuDialogOptions(FaceItemsListNames)
	
	elseIf Options[0] == "FaceEquipMale"
		form[] FaceItemsList = Config.GetFaceItems()
		string[] FaceItemsListNames = Utility.CreateStringArray(FaceItemsList.Length + 1)
		int i = FaceItemsListNames.Length
	;	Log("FaceEquipMale "+FaceItemsList.Length+"; "+i)
		FaceItemsListNames[0] = "( NONE )"
		while i > 1
			i -= 1
			FaceItemsListNames[i] = GetItemName(FaceItemsList[i - 1])
		endWhile
		Form EquipmentPhase = Expression.GetEquipmentPhase(Phase, Male)
		if EquipmentPhase
			SetMenuDialogStartIndex(FaceItemsList.Find(EquipmentPhase) + 1)
		else
			SetMenuDialogStartIndex(0)
...
event OnMenuAcceptST(int i)
	...
	if Options[0] == "FaceEquipFemale"
		if i >= 0
			if i > 0
				If Config.MirrorPress(Config.AdjustStage) && ShowMessage("$SSL_WarnRemoveFaceItem{"+GetItemName(Config.GetFaceItems()[i - 1])+"}", true, "$Yes", "$No")
					Config.RemoveFaceItem(Config.GetFaceItems()[i - 1])
					Expression.SetEquipmentPhase(Phase, Female, none)
					Expression.SetLipFixedPhase(Phase, Female, 0)
				Else
					Expression.SetEquipmentPhase(Phase, Female, Config.GetFaceItems()[i - 1])
					Expression.SetLipFixedPhase(Phase, Female, 1)
				endIf
			else
				Expression.SetEquipmentPhase(Phase, Female, none)
				Expression.SetLipFixedPhase(Phase, Female, 0)
			endIf
			SetMenuOptionValueST(GetItemName(Expression.GetEquipmentPhase(Phase, Female)))
			SetToggleOptionValueST(Expression.IsLipFixedPhase(Phase, Female), false, "LipFixedFemale")
		endIf
	...
	elseIf Options[0] == "FaceEquipMale"
		if i >= 0
			if i > 0
				If Config.MirrorPress(Config.AdjustStage) && ShowMessage("$SSL_WarnRemoveFaceItem{"+GetItemName(Config.GetFaceItems()[i - 1])+"}", true, "$Yes", "$No")
					Config.RemoveFaceItem(Config.GetFaceItems()[i - 1])
					Expression.SetEquipmentPhase(Phase, Male, none)
					Expression.SetLipFixedPhase(Phase, Male, 0)
				Else
					Expression.SetEquipmentPhase(Phase, Male, Config.GetFaceItems()[i - 1])
					Expression.SetLipFixedPhase(Phase, Male, 1)
				endIf
			else
				Expression.SetEquipmentPhase(Phase, Male, none)
				Expression.SetLipFixedPhase(Phase, Male, 0)
			endIf
			SetMenuOptionValueST(GetItemName(Expression.GetEquipmentPhase(Phase, Male)))
			SetToggleOptionValueST(Expression.IsLipFixedPhase(Phase, Male), false, "LipFixedMale")
		endIf
...
event OnSelectST()
	...
	; Allow Furniture
	elseIf Options[0] == "AllowFurniture"
		Config.AllowFurniture = !Config.AllowFurniture
		SetToggleOptionValueST(Config.AllowFurniture)
		ForcePageReset()
		
	; Use Advanced Furniture System
	elseIf Options[0] == "UseAdvancedFurnitureSystem"
		Config.UseAdvancedFurn = !Config.UseAdvancedFurn
		SetToggleOptionValueST(Config.UseAdvancedFurn)
		If Config.UseAdvancedFurn
			Config.FurnitureMarkers.Setup()
		EndIf
	...
	elseIf Options[0] == "FurnitureTag"
		...
		bool value = Config.ActiveFurnitureTags.Find(Config.FurnitureTags[i])>=0
		if value
			Config.ActiveFurnitureTags = PapyrusUtil.RemoveString(Config.ActiveFurnitureTags, Config.FurnitureTags[i])
			If Config.UseAdvancedFurn
				sslFurnitureMarkers.EnableAnimObjectsByFurnitureTag(Config.FurnitureTags[i])
			endIf
		else
			Config.ActiveFurnitureTags = PapyrusUtil.PushString(Config.ActiveFurnitureTags, Config.FurnitureTags[i])
			If Config.UseAdvancedFurn
				sslFurnitureMarkers.DisableAnimObjectsByFurnitureTag(Config.FurnitureTags[i])
			endIf
		endIf
		SetToggleOptionValueST(Config.ActiveFurnitureTags.Find(Config.FurnitureTags[i])>=0)
	; Strip Editor
	elseIf Options[0] == "StripEditor"
		Form ItemRef
		if (Options[1] as int) == 0
	...
	; Toggle Use FaceItems
	elseIf Options[0] == "UseFaceItems"
		Config.UseFaceItems = !Config.UseFaceItems
		SetToggleOptionValueST(Config.UseFaceItems)
	
	; Test the Expressions on the Player
	elseIf Options[0] == "ExpressionTestPlayer"
		TestApply(PlayerRef)

	; Test the Expressions on the Target
	elseIf Options[0] == "ExpressionTestTarget"
		TestApply(TargetRef)
	...
	elseIf Options[0] == "ExpressionBeastOnly"
		Expression.ToggleTag("BeastOnly")
		SetToggleOptionValueST(Expression.HasTag("BeastOnly"))

	elseIf Options[0] == "ExpressionNoBeast"
		Expression.ToggleTag("NoBeast")
		SetToggleOptionValueST(Expression.HasTag("NoBeast"))

	elseIf Options[0] == "ImportFaceItem"
		form akWorn = PlayerRef.GetWornForm(0x80000000)
		if akWorn && ShowMessage("$SSL_WarnImportFaceItem{"+GetItemName(akWorn)+"}", true, "$Yes", "$No")
			Config.AddFaceItem(akWorn)
		endIf

	elseIf Options[0] == "LipFixedFemale"
		Expression.SetLipFixedPhase(Phase, Female, (!Expression.IsLipFixedPhase(Phase, Female)) as int)
		SetToggleOptionValueST(Expression.IsLipFixedPhase(Phase, Female))

	elseIf Options[0] == "LipFixedMale"
		Expression.SetLipFixedPhase(Phase, Male, (!Expression.IsLipFixedPhase(Phase, Male)) as int)
		SetToggleOptionValueST(Expression.IsLipFixedPhase(Phase, Male))
	...
	; NPCs Use Furniture
	elseIf Options[0] == "NPCFurniture"
		Config.NPCFurniture = sslUtility.IndexTravel(Config.NPCFurniture, 3)
		SetTextOptionValueST(Chances[Config.NPCFurniture])

	; Ask Furniture
	elseIf Options[0] == "AskFurniture"
		Config.AskFurniture = sslUtility.IndexTravel(Config.AskFurniture, 4)
		SetTextOptionValueST(BedOpt[Config.AskFurniture])

	; Restrict Fetish Tags
	elseIf Options[0] == "RestrictFetishTags"
		Config.RestrictFetishTags = !Config.RestrictFetishTags
		SetToggleOptionValueST(Config.RestrictFetishTags)

	; ZaD Filters
	elseIf Options[0] == "ManageZadFilter"
		if Config.HasZadDevice
			Config.ManageZadFilter = !Config.ManageZadFilter
		else
			Config.ManageZadFilter = false
		endIf
		SetToggleOptionValueST(Config.ManageZadFilter)

	; ZaZ Filters
	elseIf Options[0] == "ManageZazFilter"
		if Config.HasZazDevice
			Config.ManageZazFilter = !Config.ManageZazFilter
		else
			Config.ManageZazFilter = false
		endIf
		SetToggleOptionValueST(Config.ManageZazFilter)

	; Use Intimacy Circle
	elseIf Options[0] == "UseIntimacyCircle"
		Config.UseIntimacyCircle = !Config.UseIntimacyCircle
		SetToggleOptionValueST(Config.UseIntimacyCircle)

	; Reset Threads
	elseIf Options[0] == "ResetThreads"
...
event OnDefaultST()
	...
	; Allow Furniture
	elseIf Options[0] == "AllowFurniture"
		Config.AllowFurniture = Config.ActiveFurnitureTags.Length > 0
		SetTextOptionValueST(Config.AllowFurniture)
	
	; Use Advanced Furniture System
	elseIf Options[0] == "UseAdvancedFurnitureSystem"
		Config.UseAdvancedFurn = False
		SetTextOptionValueST(Config.UseAdvancedFurn)
	...
	; NPCs Use Furniture
	elseIf Options[0] == "NPCFurniture"
		Config.NPCFurniture = 1
		SetTextOptionValueST(Chances[Config.NPCFurniture])
	
	; Ask Furniture
	elseIf Options[0] == "AskFurniture"
		Config.AskFurniture = 1
		SetTextOptionValueST(BedOpt[Config.AskFurniture])
	
	; Restrict Fetish Tags
	elseIf Options[0] == "RestrictFetishTags"
		Config.RestrictFetishTags = false
		SetToggleOptionValueST(Config.RestrictFetishTags)
	
	; ZaD Filters
	elseIf Options[0] == "ManageZadFilter"
		Config.ManageZadFilter = Config.HasZadDevice
		SetToggleOptionValueST(Config.ManageZadFilter)

	; ZaZ Filters
	elseIf Options[0] == "ManageZazFilter"
		Config.ManageZazFilter = Config.HasZazDevice
		SetToggleOptionValueST(Config.ManageZazFilter)

	; Use Intimacy Circle
	elseIf Options[0] == "UseIntimacyCircle"
		Config.UseIntimacyCircle = false
		SetToggleOptionValueST(Config.UseIntimacyCircle)
	endIf
endEvent
...
function InstallMenu()
	...
	if !SexLab || !SystemAlias || !Config || !ActorLib || !ThreadLib || !ThreadSlots || !AnimSlots || !CreatureSlots || !VoiceSlots || !ExpressionSlots 
		AddTextOptionST("InstallError", "$SSL_FileIntegrityError", "README")
		SetInfoText("$SSL_WarnFileIntegrityError")
		return
	endIf
...
function SystemCheckOptions()
	AddTextOption("Skyrim Script Extender (2.1.2+)", StringIfElse(Config.CheckSystemPart("SKSE"), "<font color='#00FF00'>ok</font>", "<font color='#FF0000'>X</font>")) ;SSE
	AddTextOption("SexLabUtil.dll SKSE Plugin  (1.64+)", StringIfElse(Config.CheckSystemPart("SexLabUtil"), "<font color='#00FF00'>ok</font>", "<font color='#FF0000'>X</font>"))
	AddTextOption("PapyrusUtil.dll SKSE Plugin  (4.1+)", StringIfElse(Config.CheckSystemPart("PapyrusUtil"), "<font color='#00FF00'>ok</font>", "<font color='#FF0000'>X</font>")) ;SSE
	AddTextOption("SKEE64.dll SKSE Plugin  (3.4+)", StringIfElse(Config.CheckSystemPart("NiOverride"), "<font color='#00FF00'>ok</font>", "<font color='#0000FF'>?</font>")) ;SSE
	AddTextOption("FNIS - Fores New Idles in Skyrim (7.0+)", StringIfElse(Config.CheckSystemPart("FNIS"), "<font color='#00FF00'>ok</font>", "<font color='#FF0000'>X</font>")) ;SSE
	AddTextOption("FNIS For Users Behaviors Generated", StringIfElse(Config.CheckSystemPart("FNISGenerated"), "<font color='#00FF00'>ok</font>", "<font color='#0000FF'>?</font>"))
	AddTextOption("FNIS SexLab Framework Idles", StringIfElse(Config.CheckSystemPart("FNISSexLabFramework"), "<font color='#00FF00'>ok</font>", "<font color='#0000FF'>?</font>"))
	AddTextOption("FNIS Creature Pack (7.0+)", StringIfElse(Config.CheckSystemPart("FNISCreaturePack"), "<font color='#00FF00'>ok</font>", "<font color='#0000FF'>?</font>")) ;SSE
	AddTextOption("FNIS SexLab Creature Idles", StringIfElse(Config.CheckSystemPart("FNISSexLabCreature"), "<font color='#00FF00'>ok</font>", "<font color='#0000FF'>?</font>"))
...
function AnimationSettings()
	...
	AddSliderOptionST("ExpressionTransitSpeed","$SSL_ExpressionTransitSpeed", Config.ExpressionTransitSpeed, "{1}x")
	...
	AddToggleOptionST("UseFaceItems","$SSL_UseFaceItems", Config.UseFaceItems)
	...
	AddTextOptionST("UseXMarkerHidden","$SSL_UseXMarkerHidden", BedOpt[ClampInt(Config.UseXMarkerHidden, 0, 3)])
	AddTextOptionST("NPCHidden","$SSL_NPCsUseXMarkerHidden", Chances[ClampInt(Config.NPCHidden, 0, 2)])
	if Config.HasUtilityPlus
		AddToggleOptionST("ShowInMap","$SSL_ShowInMap", Config.ShowInMap)
		AddToggleOptionST("UseIntimacyCircle","$SSL_UseIntimacyCircle", Config.UseIntimacyCircle)
	else
		AddTextOptionST("NPCBed","$SSL_NPCsUseBeds", Chances[ClampInt(Config.NPCBed, 0, 2)])
		AddTextOptionST("AskBed","$SSL_AskBed", BedOpt[ClampInt(Config.AskBed, 0, 3)])
	endIf
	...
	if Config.HasZadDevice
		AddToggleOptionST("ManageZadFilter","$SSL_ManageZadFilter", Config.ManageZadFilter)
	endIf
	if Config.HasZazDevice
		AddToggleOptionST("ManageZazFilter","$SSL_ManageZazFilter", Config.ManageZazFilter)
	endIf
	AddToggleOptionST("RestrictFetishTags","$SSL_RestrictFetishTags", Config.RestrictFetishTags, OPTION_FLAG_NONE)
	...
	AddToggleOptionST("WaitIdles","$SSL_WaitIdles", Config.WaitIdles)
...
function FurnitureSettings()
	...
	AddTextOptionST("AskBed","$SSL_AskBed", BedOpt[ClampInt(Config.AskBed, 0, 3)])
	AddTextOptionST("NPCBed","$SSL_NPCsUseBeds", Chances[ClampInt(Config.NPCBed, 0, 2)])
	AddTextOptionST("AskFurniture","$SSL_AskFurniture", BedOpt[ClampInt(Config.AskFurniture, 0, 3)], opt)
	AddTextOptionST("NPCFurniture","$SSL_NPCsUseFurnitures", Chances[ClampInt(Config.NPCFurniture, 0, 2)], opt)
	AddToggleOptionST("UseAdvancedFurnitureSystem","$SSL_UseAdvancedFurnitureSystem", Config.UseAdvancedFurn, SexLabUtil.IntIfElse((SKSE.GetPluginVersion("PapyrusExtender") >= 2 || SKSE.GetPluginVersion("powerofthree's Papyrus Extender") >= 4), OPTION_FLAG_NONE, OPTION_FLAG_DISABLED)) ;SSE
	AddToggleOptionST("AllowFurniture","$SSL_AllowFurniture", Config.AllowFurniture)
	AddHeaderOption("$SSL_AllowedFurnitureTags")
	AddHeaderOption("")
	
	int i
	while i < Config.FurnitureTags.Length
		if Config.FurnitureTags[i]
			AddToggleOptionST("FurnitureTag_"+i, Config.FurnitureTags[i], Config.ActiveFurnitureTags.Find(Config.FurnitureTags[i])>=0, opt)
		endIf
		i += 1
	endWhile
endFunction
...
bool function KeyConflict(int newKeyCode, string conflictControl, string conflictName)
	...
	if (conflictControl != "")
		string msg
		if (conflictName != "")
			msg = "$SSL_WarnKeyMapped{"+conflictControl+"}_{"+conflictName+"}"
		else
			msg = "$SSL_WarnKeyMapped{"+conflictControl+"}"
		endIf
...
function AnimationEditor()
	...
	if !PreventOverwrite 
		...
		if !(ThreadControlled && (ThreadControlled.GetState() == "Animating" || ThreadControlled.GetState() == "Advancing" || ThreadControlled.GetState() == "Resetting"))
		...
		if ThreadControlled && ThreadControlled.Animation
			...
			FurnitureType = ThreadControlled.FurnitureTypeID
	...
	if AnimEditPage > LastPage || AnimEditPage < 1
		AnimEditPage = 1
	endIf
	...
	if AdjustKey == "Animation"
		...
		if (ControlledAnimation == Animation && FurnitureType >= 4) || Animation.HasTag("Furniture")
			...
			AnimOffsets = Animation.GetBedOffsets()
		endIf
		if !ThreadControlled && (Animation.PositionCount == 1 && (!IsCreatureEditor || (Animation.HasActorRace(PlayerRef) || (TargetRef && Animation.HasActorRace(PlayerRef))))) || (Animation.PositionCount >= 2 && TargetRef && (!IsCreatureEditor || (Animation.HasActorRace(PlayerRef) || Animation.HasActorRace(TargetRef))))
...
state AnimationSelect
	...
	event OnMenuOpenST()
		...
		endIf
	;	Log("LastPage:"+LastPage+"; PerPage:"+PerPage+"; AnimEditPage:"+AnimEditPage+"; Slots:"+AnimationSlots.Slotted)
...
state AnimationTest
	event OnSelectST()
		if ShowMessage("$SSL_WarnTestAnimation{"+Animation.Name+"}", true, "$Yes", "$No")
			sslThreadModel Thread
		;	sslThreadController Control = Config.GetThreadControlled()
			bool ChangeScene = false
		;	if Control && (Control.GetState() == "Animating" || Control.GetState() == "Advancing" || Control.GetState() == "Resetting")
		;		Thread = Control as sslThreadModel
		;		ChangeScene = True
		;	else
		...
		;	endIf
			...
			;				FindGender = -1
			;			endIf
			;			bool ValidPlayer = ((Control.Positions.Find(PlayerRef) >= 0 && (FindGender < 0 || FindGender == ActorLib.GetGender(PlayerRef))) || ThreadLib.CheckActor(PlayerRef, FindGender)) && (RaceKey == "" || sslCreatureAnimationSlots.GetAllRaceKeys(PlayerRef.GetLeveledActorBase().GetRace()).Find(RaceKey) != -1)
			;			bool ValidTarget = ((Control.Positions.Find(TargetRef) >= 0 && (FindGender < 0 || FindGender == ActorLib.GetGender(TargetRef))) || ThreadLib.CheckActor(TargetRef, FindGender)) && (RaceKey == "" || sslCreatureAnimationSlots.GetAllRaceKeys(TargetRef.GetLeveledActorBase().GetRace()).Find(RaceKey) != -1)
			;			if ValidPlayer && ValidTarget
			;				if ShowMessage("$SSL_WarnTestAnimation{"+Animation.Name+"}ActorChose", true, TargetName, PlayerName)
			;					Control.ChangeActors(sslUtility.MakeActorArray(TargetRef))
			;				else
			;					Control.ChangeActors(sslUtility.MakeActorArray(PlayerRef))
			;				endIf
			;			elseIf ValidTarget
			;				Control.ChangeActors(sslUtility.MakeActorArray(TargetRef))
			;			elseIf ValidPlayer
			;				Control.ChangeActors(sslUtility.MakeActorArray(PlayerRef))
			;			else
			;				ShowMessage("$SSL_WarnTestAnimationActorError", false)
			;			endIf
			;		else
			;			bool ValidPlayer = ((Control.Positions.Find(PlayerRef) >= 0 && (FindGender < 0 || Animation.Genders[ActorLib.GetGender(PlayerRef)] > 0)) || ThreadLib.CheckActor(PlayerRef, FindGender)) && (RaceKey == "" || sslCreatureAnimationSlots.GetAllRaceKeys(PlayerRef.GetLeveledActorBase().GetRace()).Find(RaceKey) != -1)
			;			bool ValidTarget = ((Control.Positions.Find(TargetRef) >= 0 && (FindGender < 0 || FindGender == ActorLib.GetGender(TargetRef))) || ThreadLib.CheckActor(TargetRef, FindGender)) && (RaceKey == "" || sslCreatureAnimationSlots.GetAllRaceKeys(TargetRef.GetLeveledActorBase().GetRace()).Find(RaceKey) != -1)
			;			if ValidPlayer && ValidTarget
			;				if ShowMessage("$SSL_WarnTestAnimation{"+Animation.Name+"}ActorChose", true, TargetName, PlayerName)
			;					Control.ChangeActors(sslUtility.MakeActorArray(TargetRef))
			...
			;			endIf
			;			actor[] Positions = 
			;		;	Control.ChangeActors(ThreadLib.FindAnimationPartners(Animation, Thread.CenterRef, 1500, PlayerRef, TargetRef))
			;			Control.ChangeActors(ThreadLib.FindAvailablePartners(Animation, Thread.CenterRef, 1500, PlayerRef, TargetRef))
			;		endIf
			;	else
			...
			;	endIf
				if Animation.PositionCount != Thread.ActorCount
					ShowMessage("$SSL_WarnTestAnimationActorCountError{"+Animation.PositionCount+"}_{"+Thread.ActorCount+"}", false)
				else
					ShowMessage("$SSL_WarnStartingAnimation{"+Animation.Name+"}", false)
					Utility.Wait(0.5)
					if !ChangeScene && !Thread.StartThread()
						ShowMessage("$SSL_WarnTestAnimationError", false)
					endIf
				endIf
			else
				ShowMessage("$SSL_WarnTestAnimationError", false)
	...
	endEvent
	event OnHighlightST()
		SetInfoText("$SSL_InfoAnimationTest")
	endEvent
endState
...
function ToggleAnimations()
	...
	if LastPage < 4
		...
		AddSliderOptionST("AnimationTogglePage", "Page #", TogglePage, "{0} / "+LastPage)
	endIf

	;if AllowTagToggle
		AddMenuOptionST("FilterByTag", "$SSL_FilterByTag", StringIfElse(!TagFilter, "---", TagFilter), DoDisable(!AllowTagToggle))
		AddMenuOptionST("FilterAction", "$SSL_Action", TFAction[TFA], DoDisable(TFAction.Length < 2))
...
state AnimationTogglePage
	event OnSelectST()
		...
		if TogglePage > LastPage
			TogglePage = 1
		endIf
		SetTextOptionValueST(TogglePage+" / "+LastPage, false)
		ForcePageReset()
	endEvent
	event OnSliderOpenST()
		SetSliderDialogStartValue(TogglePage)
		SetSliderDialogDefaultValue(1)
		SetSliderDialogRange(1, LastPage)
		SetSliderDialogInterval(1)
	endEvent
	event OnSliderAcceptST(float value)
		SetOptionFlagsST(OPTION_FLAG_DISABLED)
		TogglePage = value as Int
		SetSliderOptionValueST(TogglePage, "{0} / "+LastPage)
		ForcePageReset()
	endEvent
	event OnDefaultST()
		SetOptionFlagsST(OPTION_FLAG_DISABLED)
...
function ExpressionEditor()
	...
	if EditOpenMouth
		...
		AddSliderOptionST("MouthScalePlayer","$SSL_MouthScalePlayer", (StorageUtil.GetFloatValue(PlayerRef, "SexLab.MouthScale", 1.0) * 100) as Int, "{0}%")
		AddSliderOptionST("MouthScaleTarget","$SSL_MouthScale{"+TargetName+"}", (StorageUtil.GetFloatValue(TargetRef, "SexLab.MouthScale", 1.0) * 100) as Int, "{0}%", SexLabUtil.IntIfElse((!TargetRef), OPTION_FLAG_DISABLED, OPTION_FLAG_NONE))
	...
	; 5
	AddToggleOptionST("ExpressionBeastOnly", "$SSL_ExpressionBeastOnly", Expression.HasTag("BeastOnly"))
	AddTextOptionST("ExpressionTestTarget", "$SSL_TestOn{"+TargetName+"}", "$SSL_Apply", SexLabUtil.IntIfElse(!TargetRef, OPTION_FLAG_DISABLED, OPTION_FLAG_NONE))

	; 6
	AddToggleOptionST("ExpressionNoBeast", "$SSL_ExpressionNoBeast", Expression.HasTag("NoBeast"))
	AddTextOptionST("ImportFaceItem", "$SSL_ImportFaceItem", "$SSL_ClickHere", SexLabUtil.IntIfElse(!PlayerRef.GetWornForm(0x80000000), OPTION_FLAG_DISABLED, OPTION_FLAG_NONE)) ; Get Slot 44 on player

	; 7
	AddHeaderOption("$SSL_PhaseConfig")
	AddHeaderOption("")
	...
	; Extra settings
	AddHeaderOption("$SSL_{$SSL_Female}-{$SSL_ExtraEffects}", FlagF)
	AddHeaderOption("$SSL_{$SSL_Male}-{$SSL_ExtraEffects}", FlagM)

	AddMenuOptionST("FaceEquipFemale", "$SSL_FaceEquip", GetItemName(Expression.GetEquipmentPhase(Phase, Female)), FlagF)
	AddMenuOptionST("FaceEquipMale", "$SSL_FaceEquip", GetItemName(Expression.GetEquipmentPhase(Phase, Male)), FlagM)

	AddToggleOptionST("LipFixedFemale", "$SSL_LipFixed", Expression.IsLipFixedPhase(Phase, Female), FlagF)
	AddToggleOptionST("LipFixedMale", "$SSL_LipFixed", Expression.IsLipFixedPhase(Phase, Male), FlagM)
...
function TestApply(Actor ActorRef)
	...
	if EditOpenMouth
		if ShowMessage("$SSL_WarnTestExpression{"+ActorName+"}", true, "$Yes", "$No")
			...
			Utility.Wait(0.1)
			if ActorRef == PlayerRef && Game.GetCameraState() != 8 && Game.GetCameraState() != 3
	...
	elseIf Expression && ShowMessage("$SSL_WarnTestExpression{"+ActorName+"}", true, "$Yes", "$No")
		...
		if !ShowMessage("$SSL_WarnTestExpressionWithOpenMouth", true, "$No", "$Yes")
			testOpenMouth = true
		endIf
		...
		if testOpenMouth
			...
			if !ActorRef.IsInFaction(Config.AnimatingFaction)
...
function TimersStripping()
	...
	if Biped.Length > 33 ; Ammo
		if Strip1.Length > 33
			AddToggleOptionST("Stripping_1_33", Biped[33], Strip1[33])
		endIf
		if Strip0.Length > 33
			AddToggleOptionST("Stripping_0_33", Biped[33], Strip0[33])
		endIf
	endIf
	int i
	while i < 32
...
;	endIf ;Not longer need with StripEditorPossibility option
	...
	if Max < 1
		AddHeaderOption("SSL_NotifyMaxItemsDisplayReached")
	endIf
endFunction
...
function RebuildClean()
	...
	AddTextOptionST("ResetThreads","$SSL_ResetThreads", "$SSL_ClickHere")
	...
	AddHeaderOption("$SSL_RegistryInfo")
	...
	AddTextOption("$SSL_Animations{$SSL_Standard}", AnimSlots.Slotted+" / "+AnimSlots.GetNumAliases(), OPTION_FLAG_DISABLED)
	AddTextOption("$SSL_Animations{$SSL_Creature}", CreatureSlots.Slotted+" / "+CreatureSlots.GetNumAliases(), OPTION_FLAG_DISABLED)
	AddTextOption("$SSL_Voices", VoiceSlots.Slotted+" / "+SexLabUtil.IntIfElse(VoiceSlots.GetNumAliases() > 500, 375, VoiceSlots.GetNumAliases()), OPTION_FLAG_DISABLED)
	AddTextOption("$SSL_Expressions", ExpressionSlots.Slotted+" / "+SexLabUtil.IntIfElse(ExpressionSlots.GetNumAliases() > 500, 375, ExpressionSlots.GetNumAliases()), OPTION_FLAG_DISABLED)


	AddHeaderOption("$SSL_SystemRequirements")
	SystemCheckOptions()	
endFunction
...
state LimitedStrip
	...
	endEvent
endState
state UseXMarkerHidden
	event OnSelectST()
		Config.UseXMarkerHidden = sslUtility.IndexTravel(Config.UseXMarkerHidden, 4)
		SetTextOptionValueST(BedOpt[Config.UseXMarkerHidden])
	endEvent
	event OnDefaultST()
		Config.UseXMarkerHidden = 2
		SetTextOptionValueST(BedOpt[Config.UseXMarkerHidden])
	endEvent
	event OnHighlightST()
		SetInfoText("$SSL_UseXMarkerHiddenInfo")
	endEvent
endState
state NPCHidden
	event OnSelectST()
		Config.NPCHidden = sslUtility.IndexTravel(Config.NPCHidden, 3)
		SetTextOptionValueST(Chances[Config.NPCHidden])
	endEvent
	event OnDefaultST()
		Config.NPCHidden = 0
		SetTextOptionValueST(Chances[Config.NPCHidden])
	endEvent
	event OnHighlightST()
		SetInfoText("$SSL_NPCsUseXMarkerHiddenInfo")
	endEvent
endState
state RedressVictim
	event OnSelectST()
...
state AskBed
	event OnSelectST()
		Config.AskBed = sslUtility.IndexTravel(Config.AskBed, 4)
...
state ScaleActors
	event OnSelectST()
		...
		if Config.ScaleActors && Config.DisableScale
			...
			SexLabUtil.VehicleFixMode(0) ;SSE
		endIf
		ForcePageReset()
	endEvent
	event OnDefaultST()
...
state DisableScale
	event OnSelectST()
		...
		SexLabUtil.VehicleFixMode((Config.DisableScale as int)) ;SSE
	...
	endEvent
	event OnDefaultST()
		Config.DisableScale = !Config.HasRaceScale
		SexLabUtil.VehicleFixMode(0) ;SSE
		SetToggleOptionValueST(Config.DisableScale)
	endEvent
	event OnHighlightST()
...
state WaitIdles
	event OnSelectST()
		Config.WaitIdles = !Config.WaitIdles
		SetToggleOptionValueST(Config.WaitIdles)
	endEvent
	event OnDefaultST()
		Config.WaitIdles = false
		SetToggleOptionValueST(Config.WaitIdles)
	endEvent
	event OnHighlightST()
		SetInfoText("$SSL_InfoWaitIdles")
	endEvent
endState
state RagdollEnd
	event OnSelectST()
...
state ExpressionDelay
	...
	endEvent
endState
state ExpressionTransitSpeed
	event OnSliderOpenST()
		SetSliderDialogStartValue(Config.ExpressionTransitSpeed)
		SetSliderDialogDefaultValue(0)
		SetSliderDialogRange(0.0, 4)
		SetSliderDialogInterval(0.2)
	endEvent
	event OnSliderAcceptST(float value)
		Config.ExpressionTransitSpeed = value
		SetSliderOptionValueST(Config.ExpressionTransitSpeed, "{1}x")
	endEvent
	event OnDefaultST()
		Config.ExpressionTransitSpeed = 0.0
		SetSliderOptionValueST(Config.ExpressionTransitSpeed, "{1}x")
	endEvent
	event OnHighlightST()
		SetInfoText("$SSL_InfoExpressionTransitSpeed")
	endEvent
endState
state ShakeStrength
	event OnSliderOpenST()
